...
|type
|Restrict a search to an entity type, one of {term,property,individual}
...
|local
|Set to true to only return terms that are in a defining ontology e.g. Only retun matches to gene ontology terms in the gene ontology, and exclude ontologies where those terms are also referenced
