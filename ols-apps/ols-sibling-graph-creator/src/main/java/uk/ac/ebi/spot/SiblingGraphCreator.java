...
public class SiblingGraphCreator implements CommandLineRunner {

    @Autowired
    private PropertyBasedLoadingService propertyBasedLoadingService;

    @Override
    public void run(String... args) throws OntologyLoadingException, IOException {
        ...
        for (IRI classTerm : loader.getAllClasses()) {
            String classTermString = classTerm.toString();

            String termId = classTermString.substring(classTermString.lastIndexOf('/') + 1, classTermString.length());

           File file = new File("/Users/catherineleroy/Documents/json-graphs/" + termId + ".json");
            OutputStream outputStream = new FileOutputStream(file);

            buildBpopGraph(loader, classTerm,outputStream);
